...
function IframeUploader(form) {
  ...
  this.$iframe = $('<iframe id="' + this.iframeName + '" name="' + this.iframeName + '" style="display:none"></iframe>').appendTo(form);
...
IframeUploader.prototype.upload = function (fileField, url) {
  ...
  var poller = poll(function () {
    try {
      ...
      var msg = 'Could not read contents of iframe. Please verify that "' + url + '" is the correct URL. Original error was: ' + e.message;
